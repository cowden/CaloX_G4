...
G4VPhysicalVolume* CXDetectorConstruction::DefineVolumes()
{
  ...
  auto layerThickness = 2.0*cm;
  int  nofLayers = int(150*cm/layerThickness);
  double calorSizeXY  = 100.*cm;
  double sensorThickness=0.3*cm;
  ...
  auto calorMaterial = G4Material::GetMaterial("G4_Cu");
  // auto calorMaterial = G4Material::GetMaterial("G4_Si");
  // auto calorMaterial = G4Material::GetMaterial("G4_W");
  auto sensorMaterial = G4Material::GetMaterial("G4_Si");
  ...
  auto SensorS
    = new G4Box("Sensor",             // its name
                 calorSizeXY/2, calorSizeXY/2, sensorThickness/2); // its size
  ...
  new G4PVPlacement(
                 0,                // no rotation
                 G4ThreeVector(0., 0., 0.), // its position (center of absober
  ...
  G4cout
    ...
    << layerThickness/mm << "mm of (layer) " << calorMaterial->GetName() 
    << " + "
    << calorThickness/mm << "mm of (calor) " << calorMaterial->GetName() << " ] " << G4endl
    << "------------------------------------------------------------" << G4endl;
